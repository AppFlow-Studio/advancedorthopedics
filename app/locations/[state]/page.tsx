    ...
    <main className='w-full flex flex-col items-center justify-center bg-white h-full pb-10'>
      <section className="w-full h-full flex flex-col relative overflow-hidden [mask-composite:intersect] [mask-image:linear-gradient(to_top,transparent,black_6rem)]" id="Locations">
        ...
        <Image
          ...
          alt={`Find Mountain Spine & Orthopedics clinic locations in ${stateInfo?.name}`}
        />
        <div
        ...
        <div className="z-[2] flex flex-col w-full h-full text-left relative pt-32 lg:pt-26 pb-20 px-6 lg:px-[80px]">
          <div className='max-w-[1440px] w-full flex flex-col items-start justify-start'>
            ...
            >
              {`${stateInfo?.name || state.toUpperCase()} Locations`}
            </TextAnimate>
            <h2
              style={{
                fontFamily: 'var(--font-reem-kufi)',
            ...
            >
              Find Orthopedic Care in {stateInfo?.name}
            </h2>
            <p
              style={{
                fontFamily: 'var(--font-reem-kufi)',
            ...
            >
              Mountain Spine & Orthopedics delivers expert spine care across {stateInfo?.name} with {stateClinics.length} location{stateClinics.length > 1 ? 's' : ''}.
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="bg-white w-full">
        ...
        >
          ...
          <p className="text-lg md:text-xl text-[#424959] max-w-3xl mx-auto">
            Visit any of our state-of-the-art facilities in {stateInfo?.name} for expert orthopedic care and personalized treatment.
          </p>
        </motion.div>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="text-center">
              ...
              <h3 className="text-xl font-bold text-[#252932] mb-2">{stateClinics.length} {stateInfo?.name} Location{stateClinics.length > 1 ? 's' : ''}</h3>
              <p className="text-[#424959]">Serving communities across {stateInfo?.name}</p>
            </div>
          </div>
        </motion.div>
        ...
        <div className="mt-12 text-center">
          <h3 className="text-2xl font-bold text-[#252932] mb-6">Explore Other States</h3>
          <div className="flex flex-wrap justify-center gap-4">
            {VALID_STATE_SLUGS.filter(s => s !== state).map(otherState => {
              const otherStateInfo = STATE_METADATA[otherState]
              return (
                <Link
                  key={otherState}
                  href={`/locations/${otherState}`}
                  className="px-6 py-3 bg-white border border-[#0A50EC]/20 rounded-full text-[#0A50EC] font-medium hover:bg-[#0A50EC] hover:text-white transition-all duration-300"
                >
                  {otherStateInfo?.name}
                </Link>
              )
