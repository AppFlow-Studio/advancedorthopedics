...
const neckAndShoulderConditions: ConditionInfoProp[] = painconditions.filter(x => 
  ...
  x.slug == 'neck-herniated-disc' || 
  x.slug == 'cervical-degenerative-disc-disease' || 
  x.slug == 'neck-shoulder-arthritis-pain' || 
  x.slug == 'pinched-nerve-neck-shoulder'
)

const neckAndShoulderTreatment: ConditionInfoProp[] = PainAreaTreatments.filter(x => 
  x.slug == 'neck-and-shoulder-pain-treatment'
)

const neckAndShoulderOptions: ConditionInfoProp[] = [...neckAndShoulderConditions, ...neckAndShoulderTreatment]
    ...
    <div className="w-full max-w-md">
      ...
      >
        <div ref={contentRef} className="rounded-md space-y-[20px] mt-[20px] pb-[20px]">
          {neckAndShoulderOptions.map((option) => {
