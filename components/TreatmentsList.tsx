    ...
    <div className="w-full max-w-lg mx-auto space-y-10"> {/* Optional: Container styling */}
       ...
       <div className=' flex flex-col space-y-[20px] hover:cursor-pointer mt-[32px]'>
        {data.map((treatment, index) => {
          ...
          return (
            <Link href={`/treatments/${treatment.slug}`} key={treatment.title} className={`${currentTreatment == treatment.title ? 'bg-[#022968] text-white' : 'bg-[#FAFAFA] text-[#111315]'} p-[16px] w-full flex flex-row justify-between items-center rounded-[10px] ${visibilityClasses}`} >
                <h1
                style={{
