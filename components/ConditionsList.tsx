           ...
           {data.map((condition: ConditionInfoProp, index: number) => {
             ...
             return (
               <li key={condition.title} className={visibilityClasses}>
                 <Link href={`/area-of-specialty/${condition.slug}`} className={`${currentCondition == condition.title ? 'bg-[#022968] text-white' : 'bg-[#FAFAFA] text-[#111315]'} p-[16px] w-full flex flex-row justify-between items-center rounded-[10px]`}>
                   <span
                     style={{
