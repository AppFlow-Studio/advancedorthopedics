      ...
      >
        <div ref={contentRef} className="rounded-md space-y-[20px] mt-[20px]">
          {footPainOpitions.map((option) => (
            <Link href={`/area-of-pain/foot-pain/${option.slug}`} className={`${CurrentCondition == option.title ? 'bg-[#FAFAFA]' : 'bg-[#FAFAFA]'} p-[16px] w-full flex flex-row justify-between items-center rounded-[10px]`} key={option.title}>
              <span
                style={{
                  fontFamily: "var(--font-public-sans)",
                  fontWeight: 400,
