    ...
    <div className="w-full max-w-md">
      ...
      <div
       className={cn(
        ...
        isOpen ? "max-h-[700px] opacity-100 scale-y-100" : "max-h-0 opacity-0 scale-y-0",
      )}
      >
        <div ref={contentRef} className="rounded-md space-y-[20px] mt-[20px]">
          {footPainOpitions.map((option) => (
             <Link href={`/area-of-pain/foot-pain/${option.slug}`} className={`${CurrentCondition == option.title ? 'bg-[#EFF5FF]' : 'bg-[#FAFAFA]'} p-[16px] w-full flex flex-row justify-between items-center rounded-[10px]`} key={option.title}>
                <h1
                style={{
                fontFamily: "var(--font-reem-kufi)",
                fontWeight: 400,
