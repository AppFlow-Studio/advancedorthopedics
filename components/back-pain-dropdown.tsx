...
const backPainOptions: ConditionInfoProp[] = conditions.filter(x => x.title == 'Lower Back Pain' || x.title == 'Sciatica' || x.title == 'Coccydynia' || x.title == 'Lumbar Herniated Disc' || x.title == 'Degenerative Disc Disease' || x.title == 'Foraminal Stenosis')
      ...
      >
        <div ref={contentRef} className="rounded-md space-y-[20px] mt-[20px]">
          {backPainOptions.map((option) => (
            <Link href={`/area-of-pain/back-pain/${option.slug}`} className={`${CurrentCondition == option.title ? 'bg-[#FAFAFA]' : 'bg-[#FAFAFA]'} p-[16px] w-full flex flex-row justify-between items-center rounded-[10px]`} key={option.title}>
              <span
                style={{
                  fontFamily: "var(--font-public-sans)",
                  fontWeight: 400,
